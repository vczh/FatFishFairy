...
测试使用模拟模型回复和临时目录，不读取真实密钥、不截屏、不联网。覆盖记忆文件安全、配置、流式响应、输出格式、错误反馈、多轮模型流程，以及窗口位置、主题顺序、主题选择与回退、切换主题后重建精灵会话并保留记忆、设置保存、动画播放逻辑和发言记录的追加、时间格式与写入失败处理。完整测试集应通过，Debug 构建应无内存泄漏。
