...
- **FatFishFairy**：显示置顶的透明角色窗口，播放动画，支持切换主题、拖动并记住主题和位置。目前尚未接入模型，不需要 API 密钥。
...
- 启动时在窗口上方显示系统气泡“Hello, world!”，气泡的小尾巴向下指向精灵窗口顶部中央，随窗口移动并保持显示，直到程序关闭；靠近屏幕边缘时，会调整气泡位置以保持可见。
...
# 桌面窗口：透明置顶、动画、气泡跟随、拖动、位置恢复、主题菜单与切换恢复、菜单退出
& ./FatFish/UnitTest/Invoke-Fairy.ps1 -Configuration Debug -Platform x64
```

CLI 集成测试只将截图发送到本机回环测试服务器，不保存图片、不访问真实模型服务。桌面集成测试使用临时程序、主题和配置，检查主题菜单的顺序、切换保存、启动恢复及无效主题键的回退，不读取真实配置或密钥；可加上 `-ScreenshotPath PATH` 保存菜单截图，目视检查中文主题名称和勾选标记。测试会移动鼠标，结束后恢复指针位置。
