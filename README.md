...
- 精灵性格由 `themes/<当前主题>/Character.md` 决定；文件不存在时使用 `themes/loli_maid/Character.md`。每次向精灵模型发起请求前读取当前主题的性格，切换主题会保留对话和记忆；已经发起的请求仍可能按原性格发言。
...
原有性格文件已移至 `themes/loli_maid/Character.md`，内容保持不变；长大的妹抖暂未提供独立性格文件，默认沿用这一性格。`env/Character.md` 不再使用。性格文件已存在但为空或无法读取时会报错，修正文件后桌面程序会重试。
...
桌面程序持续截屏并请求配置的模型服务。截图在内存中处理，发送给视觉模型；精灵模型收到完整观察描述及该轮观察结束时的本机日期时间。视觉模型每轮使用新会话，精灵模型在进程内保留对话，并可维护 `memory` 中的记忆文件。模型请求期间仍可拖动、切换主题和退出；退出会取消未完成的请求并停止循环。
...
测试使用模拟模型回复和临时目录，不读取真实密钥、不截屏、不联网。覆盖记忆文件安全、配置、流式响应、输出格式、错误反馈、多轮模型流程，以及窗口位置、主题顺序、主题选择与回退、设置保存和动画播放逻辑。完整测试集应通过，Debug 构建应无内存泄漏。
...
两个集成测试都只将显示器截图发送到本机回环测试服务器，不保存这些截图、不访问真实模型服务。桌面集成测试使用临时程序、主题、配置和记忆目录，检查透明置顶、动画、气泡跟随、拖动与位置恢复、主题菜单顺序、切换保存、启动恢复及无效主题键的回退，也检查连续模型调用、发言更新与静默、错误显示与恢复，以及请求尚未完成时通过菜单退出。测试不读取真实配置或密钥；可加上 `-ScreenshotPath PATH` 保存测试窗口、菜单和气泡截图，目视检查中文主题名称、勾选标记和完整发言。长发言的实际屏幕截图以 `.speech-desktop.png` 结尾。测试会移动鼠标，结束后恢复指针位置。
