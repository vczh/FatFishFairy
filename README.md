...
本仓库的license不包含`themes`文件夹下的所有图片。reference.md均为网上下载，其他图片均为AI根据它们生成。
...
- 窗口左下角用天蓝色粗体显示运行进度：`V` 表示正在准备或执行视觉观察，`F` 表示精灵正在回应，`L` 表示屏幕不可用时正在休眠。普通错误的重试等待期间保留当前阶段。
- 字母后的数字表示自上次成功以来的失败次数，零时省略。上下文超限会立即累加，即使随后恢复，也会显示 `F1`、`F2` 等；其他导致本轮失败的错误计一次。下一轮可能显示 `V2`，切换主题不会清零；完整一轮成功后立即清零，静默成功也一样。
- 所有显示器都被拒绝访问，或没有活动显示器时，例如锁屏或显示器会话不可用，显示不带数字的 `L`。休眠期间保留当前气泡、精灵对话、记忆和失败次数，不请求模型或写入发言记录；后台每 60 秒自动重试截图，退出可立即取消等待。
- 部分显示器仍可截取时，按原显示器顺序发送成功的截图。只有新一次截图成功后才离开 `L`，恢复带有原失败次数的 `V`、`F`，完整一轮成功后再清零。若所有截图都失败且含有其他类型的错误，仍按普通错误在气泡报错、计入失败次数并在一秒后重试；已经休眠时继续保持 `L`。
...
内置三个主题，共 30 组动画、100 帧，可通过右键菜单切换：
...
- **纳垢灵**：绿色双角、肚子上另有一张大嘴的 Q 版小妖怪，共 10 组、30 帧，包含照料花园、搅拌小锅、摇铃、逗苍蝇、玩蘑菇、吃零食、跳舞、睡觉、编程和送礼物等动画。
...
在右键菜单中选择“主题 → 纳垢灵”，即可使用 `themes/nurgling/Character.md` 中独立的顽皮花园玩伴性格：喜欢蘑菇、泡泡和小恶作剧，以轻快中文陪伴用户，是战锤四万纳垢灵的可爱桌面改编。
...
测试使用模拟模型回复和临时目录，不读取真实密钥、不截屏、不联网。覆盖记忆文件安全、配置、流式响应、输出格式、错误反馈、多轮模型流程、上下文过长时分阶段删减与重试，以及窗口位置、主题顺序、主题选择与回退、切换主题后重建精灵会话并保留记忆、设置保存、动画播放逻辑和发言记录的追加、时间格式与写入失败处理。还覆盖部分显示器可用、全部拒绝访问、无活动显示器、`L` 休眠与自动重试、保留气泡和失败次数、休眠期间切换主题，以及取消等待和成功截图后的恢复；通过模拟等待检查一分钟重试，无需实际等待一分钟。完整测试集应通过，Debug 构建应无内存泄漏。

### 本地集成测试

运行常规集成测试和真实模型验证时，请保持 Windows 桌面可见且已解锁。若屏幕捕获被拒绝，例如出现 `BitBlt failed (Windows error 5)`，先检查当前桌面会话，再重试。下方单独的锁屏休眠测试需要保持锁屏。
...
此可选测试需要 Debug x64 程序及其匹配的 PDB，以及 Windows SDK 的 x64 CDB 调试器。先手动锁定 Windows，并在测试结束前保持锁屏；脚本不会锁定或解锁电脑，也不会人为制造截图失败。在 PowerShell 7 中运行：

```powershell
& ./FatFish/UnitTest/Invoke-FairyRest.ps1
```

可用 `-CdbPath PATH` 指定调试器，或用 `-EvidenceDirectory PATH` 指定证据目录。测试使用独立临时配置和本机回环监听器，不读取真实密钥；验证两次真实的 `BitBlt` 访问拒绝、两次截图尝试间隔约 60 秒、全程没有 HTTP 请求、UI 标签文字持续保持不带数字的 `L`、保留启动问候，以及在下一次休眠期间两秒内正常退出。

CDB 记录传给实际 UI 标签回调的文字；可用时还会保存原生 `PrintWindow` 渲染图。默认在 `.artifacts/capture-rest` 保留这些证据及 JSON 报告，它们不属于锁屏桌面的实际截图。解锁后仍需执行常规屏幕显示验证。
