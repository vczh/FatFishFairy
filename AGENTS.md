...
- Run one round at a time on an owned background worker. Reuse the same `FairyApplication` to retain the fairy conversation and memory; each vision observation still starts a new session.
