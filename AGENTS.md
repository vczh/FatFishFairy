...
`themes/nurgling/Character.md` provides the Nurgling's independent Chinese personality: a mischievous, cheerful garden playmate inspired by Warhammer 40,000. It uses `我` (occasionally `小纳垢灵`) for itself and `你` or `园友` for the user, without carrying over either maid's identity after a theme switch. Its background research and the rationale for this cute desktop adaptation are in `themes/nurgling/Character.research.md`, which is documentation and must not be loaded as a character prompt.
...
- Offline capture tests must cover partial success in monitor enumeration order, access denied on every monitor, no active monitors, unrelated capture failures and cancellation. Worker tests use an injected wait to verify repeated 60-second retries without model requests, bubble publication or speech-history writes; preserved conversation, memory and failure counts; theme changes during rest; recovery only after a fresh successful capture; and ordinary one-second failures while already resting. Also verify cancellation of the default 60-second wait without waiting a minute, cancellation from capture observers, and detached callbacks when an application outlives its worker.
